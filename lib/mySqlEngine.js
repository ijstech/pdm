...
module.exports = (function(){
	TMySQLEngine = TSQLEngine.extend({
		...
		_applyInsert: function(query, callback){
			...
			for (var v in fields){
				var field = fields[v];
				if (field.fieldType == 9){ // {createDate}
					sysDateFields.push(field.fieldName)
				}
				else if (field.fieldType == 11){ // {modifyDate}				
					sysDateFields.push(field.fieldName);
				}
			}
			var self = this;
			function _applySQL(records, idx){
				...
				if (item){
					...
					var values = [];
					for (var n in item){
						if (n != '_uid' && fields[n] && sysDateFields.indexOf(n) < 0){
							var v  = item[n]
					...
					for (var k = 0; k < sysDateFields.length; k++){
						col = col + ',`' + sysDateFields[k] + '`';
						value = value +',NOW()';
					}
			...
			function _applySQL(records, idx){
				...
				if (item){
					...
					for (var k = 0; k < values.length; k ++){
						var n = values[k]
						if (fields[n] && sysDateFields.indexOf(n) < 0){
							if (skipFields.indexOf(n) < 0){
					...
					for (var k = 0; k < sysDateFields.length; k ++){						
						if (value == ''){
							value = ' SET `' + sysDateFields[k] + '`= NOW()' 
						}
						else{
							value = value + ',`' + sysDateFields[k] + '`= NOW()'
						}
					}
